...
class IncrementExecutionSpec extends ApiSpecsDefaults {
  ...
  "PUT /workflows/<workflow_id>/executions/<workflow_execution_id> with empty body" should "increment current step " +
  ...
  it should "return 400 BAD REQUEST otherwise" in {
    ...
    workflows.set(id, new Workflow(id, 10))
    executions.set(eid, new WorkflowExecution(id, eid, 9 /* last step */))
